...
(function () {
    function $one(el, context) {
        return (context || document).querySelector(el);
    }

    function $all(el, context) {
    ...
    class BasicMenu {
        ...
        constructor(addons) {
            let _ = this;
            let _wrapper = $one(".menu");

            _.def = {
                wrapper: _wrapper,
                anchors: $all(":scope > .menu_item", _wrapper),
                line: $one(":scope > .menu_line", _wrapper),
                underline: $one(":scope > .menu_underline", _wrapper),
            };
